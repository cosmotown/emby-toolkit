...
""",
...

    {
        ...
        "changelog": """## ReviewList 历史 Series 清理修复
...
- 不放宽 `episode_target_series_mismatch` 身份保护，也不新增手工输入任意 ItemID 的入口。
- 不修改 STRM Inventory、Fast Audit、ffprobe、dffmpeg、PlaybackInfo、115、神医内部实现或广域 Emby 刷新逻辑。
...
""",
    ...
    },
    {
        ...
        "changelog": """## 手机端体验与人物清理安全加固

### 人物关联核验
- `PersonIds` 列表和批量详情都无法提供可用 `People` 时，只对仍未解决的作品使用有上限的并发单作品详情兜底，兼容 Emby 只在 `/Items/{id}` 返回完整人物明细的情况。
- 默认路径仍使用批量详情避免 N+1；单作品兜底最多处理 200 部作品，超过上限或任一详情无法核验时继续失败关闭，绝不会判定为可删除。
- 连接失败、响应异常、People 不可用、精确关联、明确无关联和身份别名继续分别展示并按不同安全状态处理。

### 保护库与候选选择
- 保护库的“已保护人数”只统计去重后的 Emby Person ID，不再把姓名保护键重复加到人数中；姓名键数量单独展示。
- 没有 TMDb、IMDb 或豆瓣 ID 的人物仍可按 Emby Person ID 与规范化姓名受到保护。
- 未完成实时核对、核对失败或 People 不可用的候选不能勾选；只有明确核对为当前 Person ID 关联作品为 0 的候选才开放选择。

### 手机端界面
- 移除手机端左上角重复侧栏入口，将完整功能导航改为底部“更多”独立页面，并修复长页面无法滚动和内容被底栏裁切的问题。
- 头像菜单中的“界面布局”和“主题”合并为“主题与外观”，保留主题与布局的统一设置入口。
- 调整人物清理操作区、核对弹窗和消息提示在窄屏上的尺寸与换行，减少遮挡和重叠。
- 数据看板的分辨率饼图在手机端补充数值、占比和颜色明细，图形之外也能直接读取数据。

### 兼容与测试
- 不包含数据库结构变更，不会自动扫描或删除人物，也不会改变已有演员补充与 STRM 刷新策略。
- 完整依赖镜像中的 97 项后端回归全部通过，覆盖人物清理、TMDb/豆瓣演员去重与头像门槛、演员 SAVEPOINT 写入隔离、STRM 定向刷新和镜像迁移。
- 已在 Emby 4.9.5、神医 Pro 3.0.0.49、MoviePilot v2.15.0（linux/amd64）、P115Disk 0.2.26 与 P115StrmHelper 2.8.66 的脱敏隔离环境完成只读联调。
...
""",
    },
    {
        ...
        "changelog": """## STRM 作品目录定点刷新修复

### 修复
- 将同一作品批量新增的 STRM 文件聚合为一个作品目录通知，避免每个分集分别触发 Emby 刷新。
- 电视剧、动漫、纪录片、特摄剧以及目录化电影，统一按媒体库下的作品文件夹进行定点通知。
- 多部作品同时入库时分别通知各自作品目录，不会将电视剧、电影等媒体库根目录作为刷新目标。
- 无法匹配 Emby 媒体库根目录的 STRM 路径不再退回单文件批量通知，而是保持待确认并进入有限重试。
- STRM 入库锚点刷新改为非递归，避免刷新整季、整部剧或整个媒体库。
- Webhook 未取得具体分集 ID 时仅非递归刷新当前剧集容器，不再默认递归刷新全部子项目。

### 测试
- 新增作品目录聚合测试，覆盖同一剧集多集、多部剧集以及目录化电影。
- 更新 STRM 锚点和 Webhook 刷新测试，明确验证所有相关刷新均使用非递归模式。
""",
    },
    {
        "version": "v7.2.0",
...
### 修复
- 受保护媒体库人物继续以 Emby Person ID 为第一安全依据，并在候选写入、候选读取和实际删除三层执行硬排除。
- 对 `1①めぐみ`、`2梅田` 等带异常序号的重复 Person 仅扩大保护范围，不作为删除依据；保存保护设置后立即刷新候选列表。
- 自动演员补充统一按 TMDb/Emby 身份去重；来自 TMDb 或豆瓣的新增演员必须能够解析到有效 TMDb Person ID，并具有头像才允许写入。
- 豆瓣中文姓名和角色继续用于补充 TMDb 数据；无法经豆瓣 ID、IMDb 或 TMDb 建立可靠身份的演员不再自动新建。
- AI 仍只翻译已有姓名和角色，不能改变演员人数；带异常编号、空值、非字符串或造成姓名碰撞的译名会被拒绝并保留原名。
- 参考上游后续安全调整，移除通过 API 实时改写 Emby 全局 Person 姓名；翻译结果仅进入受控媒体元数据链路，避免改名制造人物分身。
- 演员处理完成后立即提交人物身份映射事务，避免后续元数据步骤异常导致已匹配演员数据丢失。

### 安全边界
- 不自动删除现有 Emby 人物；历史幽灵人物仍需在人物清理页完成只读扫描和人工确认。
- 不要求清空演员映射数据；清空映射反而可能降低旧人物识别能力。
- 本版从独立干净工作树构建，不包含正在开发中的其他 UI 改动。
""",
    },
    {
        "version": "v7.1.14",
